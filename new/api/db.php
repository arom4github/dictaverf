		...
		//$search = searchRequest($json);
				if(strlen($chr)>0) {
					...
					if(strtolower($chr[0]) == 'o'){
						$search .= "AND  lower(dict.word) SIMILAR  TO '((un|une|le|la|les) )*(o|ô|œ){$rest}%' ";
					}else
					...
					if(strtolower($chr[0]) == '?'){
						$search .= "AND  lower(dict.word)  NOT SIMILAR  TO ".
							"'(1|2|3|4|5|6|7|8|9|0|a|à|â|b|c|ç|d|e|é|è|ê|f|g|h|i|î|j|k|l|m|n|o|ô|œ|p|q|r|s|t|u|û|v|w|x|y|z)%' ";
					}else { 
	...
	if ($conn) {
		...
		if ($filter->getMethod()=="letter" && strlen($methodParam) > 0) {
			...
			if (strtolower($methodParam[0]) == 'u') {
				...
				"lower(resp.word) not similar to '(un|une) (1|2|3|4|5|6|7|8|9|0|a|à|â|b|c|ç|d|e|é|è|ê|f|g|h|i|î|j|k|l|m|n|o|ô|œ|p|q|r|s|t|v|w|x|y|z)%') ";
			} else
			if (strtolower($methodParam[0]) == 'l') {
				$search_char .= "AND  (lower(resp.word_inv) similar to '((un|une|le|la|les) )*l{$rest}%' AND ".
				"lower(resp.word) not similar to '(la|le|les) (1|2|3|4|5|6|7|8|9|0|a|à|â|b|c|ç|d|e|é|è|ê|f|g|h|i|î|j|k|m|n|o|ô|œ|p|q|r|s|t|u|û|v|w|x|y|z)%') ";
			} else
