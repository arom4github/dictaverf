...
if(!isset($_SESSION['test_id']) || ($_SESSION['test_id']<13 && $_SESSION['test_id']>19)){
	echo "<html><body onLoad=\"window.location='/mkdict_50/redir.php?t=new543FR'\"></body></html>";
	return;
}

if($_SESSION['test_id'] == 36) $_SESSION['i_lang'] = 'fr'; // special case
...
if($_SESSION['stage'] == 2){
		...
		$_SESSION['times']++;
		if($_SESSION['times'] > $repeat) $_SESSION['stage'] = 3;
	}else
		$_SESSION['times'] = 1;
	if(isset($_POST['words'])){
		...
		if($_SESSION['stage'] == 2){ //participate up to $repeat times
			collect_form();
			write_log("Print collection form. ".$_SESSION['times']);
		}
	}else{
		collect_form();
		write_log("Print collection form (1st times). ".$_SESSION['times']);
	}
}
